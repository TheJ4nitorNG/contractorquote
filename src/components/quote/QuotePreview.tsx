...
export const QuotePreview = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  return (
    <div ref={ref} className="bg-white p-8 sm:p-12 text-black max-w-4xl mx-auto shadow-sm min-h-[1056px] print:shadow-none print:p-0">
      {/* Header */}
      <div className="flex justify-between items-start border-b pb-8 mb-8">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 tracking-tight">{company.name || "Company Name"}</h1>
        ...
        <div className="text-right">
          <h2 className="text-2xl font-semibold text-gray-400 uppercase tracking-widest">Quote</h2>
      ...
      {/* Client Info */}
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-gray-900 border-b pb-2 mb-4">Prepared For:</h3>
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="border-b border-gray-100">
              ...
              <td className="py-4 text-gray-800 text-right">{formatCurrency(item.total)}</td>
            </tr>
          ))}
          {items.length === 0 && (
            <tr>
              <td colSpan={4} className="py-8 text-center text-gray-400 italic">No items added to this quote yet.</td>
            </tr>
          )}
        </tbody>
      </table>
